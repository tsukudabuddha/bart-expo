...
const useStationsContext = (): StationsContextValue => {
  const { isLoading, isError, data, error } = useQuery<AllStationsResponse, Error>('stations', async () => {
    const { data } = await axios.get<AllStationsResponse>('https://api.bart.gov/api/stn.aspx?cmd=stns&key=MW9S-E7SL-26DU-VV8V&json=y')
    return data
  })
